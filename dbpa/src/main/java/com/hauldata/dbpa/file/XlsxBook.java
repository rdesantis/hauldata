...
public abstract class XlsxBook extends Book {
	...
	public static void registerHandler(String name) {
		...
		Book.Factory targetBookFactory = new Book.Factory() {
			public Book instantiate(Node.Owner owner, Object path) { return new XlsxTargetBook((File.Owner)owner, (Path)path); }
			public String getTypeName() { return typeName(); }
		};
		Sheet.Factory targetSheetFactory = new Sheet.Factory() {
			public Sheet instantiate(Node.Owner book, Object name) { return new XlsxTargetSheet((Book)book, (String)name); }
			public String getTypeName() { return XlsxSheet.typeName(); }
		};

		Book.Factory sourceBookFactory = new Book.Factory() {
			public Book instantiate(Node.Owner owner, Object path) { return new XlsxSourceBook((File.Owner)owner, (Path)path); }
			public String getTypeName() { return typeName(); }
		};
		Sheet.Factory sourceSheetFactory = new Sheet.Factory() {
			public Sheet instantiate(Node.Owner book, Object name) { return new XlsxSourceSheet((Book)book, (String)name); }
			public String getTypeName() { return XlsxSheet.typeName(); }
		};
