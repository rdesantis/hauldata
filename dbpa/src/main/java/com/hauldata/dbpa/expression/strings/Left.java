...
public class Left extends Expression<String> {

	private Expression<String> string;
	private Expression<Integer> length;

	public Left(Expression<String> string, Expression<Integer> length) {
	...
	public String evaluate() {
		...
		if (lengthValue < 0) {
			throw new RuntimeException("Negative length for LEFT");
		}

		int startIndex = 0; 
		int endIndex = Math.min(stringValue.length(), lengthValue);
